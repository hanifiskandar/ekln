...
export default defineEventHandler(async (event) => {
  ...
  const ref_hr_api_url = config.private.refHrApiUrl;
  ...
  const EXTERNAL_API_URL = `http://${ref_hr_api_url}/${route}`;
  ...
  try {
    ...
    // console.log('response ssr',response)

    let status = response.status;

    if (response.status >= 200 && response.status < 300) {
      console.log('Masuk dah sini - ssr')
      return { 
          data: response.data, 
          success: true,
